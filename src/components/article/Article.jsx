...
export default function Article({ post }) {
  const contentPoints = post.content.split("\n\n"),
    contentHeaders = contentPoints
      .filter((point) => point.startsWith("##"))
      .map((header) => header.slice(3, header.length)),
    contentDesc = contentPoints.filter((point) => !point.startsWith("##")),
    { category: thisCategory, slug: thisSlug } = post;
  contentDesc.shift();
  useEffect(() => {
        ...
        <div className="grid lg:grid-cols-[1fr_300px] gap-12">
          <div className="order-2 lg:order-1">
            ...
            <div className="prose-custom">
              ...
              {contentHeaders.map((pointHeader, i) => (
                <Fragment key={pointHeader}>
                  <h2
                    id={`section-${i}`}
                  ...
                  >
                    <span className="flex items-center justify-center w-10 h-10 bg-orange-500/10 rounded-xl border border-orange-500/30">
                      <i className="fa-solid fa-camera fa-xs text-orange-500"></i>
                    </span>
                    {pointHeader}
                  </h2>
                  <p className="text-neutral-300 leading-relaxed mb-6 text-lg">
            ...
            <div className="mt-6 p-6 bg-[#111111] rounded-2xl border border-[#262626]">
              <div className="flex items-center justify-between flex-wrap gap-4">
                ...
                <div className="flex gap-2">
                  <button className="w-11 h-11 bg-[#1a1a1a] border border-[#262626] rounded-xl flex items-center justify-center text-neutral-400 hover:bg-[#1da1f2] hover:text-white hover:border-transparent transition-all duration-300 cursor-pointer">
                    <i className="fa-brands fa-x-twitter"></i>
                  </button>
                  <button className="w-11 h-11 bg-[#1a1a1a] border border-[#262626] rounded-xl flex items-center justify-center text-neutral-400 hover:bg-[#0077b5] hover:text-white hover:border-transparent transition-all duration-300 cursor-pointer">
                    <i className="fa-brands fa-linkedin-in"></i>
                  </button>
                  <button className="w-11 h-11 bg-[#1a1a1a] border border-[#262626] rounded-xl flex items-center justify-center text-neutral-400 hover:bg-[#25d366] hover:text-white hover:border-transparent transition-all duration-300 cursor-pointer">
                    <i className="fa-brands fa-whatsapp"></i>
                  </button>
                  <button className="w-11 h-11 bg-[#1a1a1a] border border-[#262626] rounded-xl flex items-center justify-center text-neutral-400 hover:bg-orange-500 hover:text-white hover:border-transparent transition-all duration-300 cursor-pointer">
        ...
        <div className="mt-20 pt-12 border-t border-[#262626]">
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {posts
              .filter(
                (post) =>
                  post.category === thisCategory && post.slug !== thisSlug,
              )
              .map(
                (post, i) =>
                  i < 3 && (
